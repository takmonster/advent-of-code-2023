...
$comparer = function (Hand $hand1, Hand $hand2) {
    ...
    while ($character_counter < 5) {
        $one_value = $hand_1_card_values[$character_counter] === 11 ? 1 : $hand_1_card_values[$character_counter];
        $two_value = $hand_2_card_values[$character_counter] === 11 ? 1 : $hand_2_card_values[$character_counter];
...
class Hand
{
    ...
    private function isFiveOfAKind(): bool
    {
        ...
        if (count($value_counts) === 2 && array_key_exists(11, $value_counts) === true) {
            return true;
        }
        return false;
    }
    ...
    private function isFourOfAKind(): bool
    {
        ...
        $value_counts = array_count_values($values);
        $joker_count = array_key_exists(11, $value_counts) ? $value_counts[11] : 0;
        if ($joker_count === 0) {
            $count_values = array_values($value_counts);
            rsort($count_values);
        ...
        // 2, 2, J, J, 3
        // 2, J, J, J, 3
        unset($value_counts[11]);
        if (count($value_counts) !== 2) {
            return false;
        }
        foreach ($value_counts as $value_count) {
            if (($value_count + $joker_count) === 4) {
                return true;
            }
        }
        return false;
    ...
    private function isFullHouse(): bool
    {
        ...
        if ($joker_count === 1) {
            unset($value_counts[11]);
            return array_values($value_counts) == [2, 2];
        }
        return false;
    }
    ...
    private function isThreeOfAKind(): bool
    {
        ...
        if ($joker_count === 1) {
            ...
            $count_values = array_values($value_counts);
            rsort($count_values);
            return $count_values == [2, 1, 1];
        }
        if ($joker_count === 2) {
            unset($value_counts[11]);
            $count_values = array_values($value_counts);
            rsort($count_values);
            return $count_values == [1, 1, 1];
        }
        return false;
    }
